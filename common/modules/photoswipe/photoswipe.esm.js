...
function getPanAreaSize(options, viewportSize /*, pswp*/) {
  return {
    x:
      viewportSize.x - (options.paddingLeft || 0) - (options.paddingRight || 0),
    y:
      viewportSize.y - (options.paddingTop || 0) - (options.paddingBottom || 0),
  };
}
...
class Slide {
  ...
  calculateZoomToPanOffset(axis, point, prevZoomLevel) {
    ...
    if (!panEdgeIsViewport && point[axis] < panPadding + this.pan[axis]) {
      ...
      !panEdgeIsViewport &&
      point[axis] > imageSize + this.pan[axis] - panPadding
    ) {
      // point is at bottom or right edge of the image
      ratio = 1;
    } else {
      ratio =
        (point[axis] - this.bounds.min[axis] - panPadding) /
...
class ScrollWheel {
  ...
  _onWheel(e) {
    ...
    if (e.ctrlKey || this.pswp.options.wheelToZoom) {
      ...
      if (currSlide.isZoomable()) {
        const destZoomLevel =
          currSlide.currZoomLevel - currSlide.zoomLevels.fit * 0.04 * deltaY;
