...
class p {
  ...
  o(t) {
    const i = this.slide['x' === t ? 'width' : 'height'] * this.currZoomLevel,
      s = 'x' === t ? 'Left' : 'Top',
      h = this.slide.pswp.options['padding' + s] || 0,
      e = this.slide.panAreaSize[t];
    (this.center[t] = Math.round((e - i) / 2) + h),
      (this.max[t] = i > e ? Math.round(e - i) + h : this.center[t]),
      (this.min[t] = i > e ? h : this.center[t]);
  }
  reset() {
    (this.center.x = 0),
      (this.center.y = 0),
...
class w {
  constructor(t, i, s) {
    (this.data = t),
      ...
      (this.zoomLevels = new u(s.options, t, i)),
      ...
      (this.pan = { x: 0, y: 0 }),
      (this.currZoomLevel = 1),
      (this.width = Number(t.w)),
      (this.height = Number(t.h)),
      (this.bounds = new p(this));
  }
  setIsActive(t) {
    t && !this.isActive
      ? this.activate()
  ...
  append(i) {
    if (((this.holderElement = i), this.data)) {
      if (
        (this._(),
        ...
        const i = this.data.msrc && this.isFirstSlide;
        (this.placeholder = t(
    ...
    } else this.holderElement.innerHTML = '';
  }
  preload() {
    (this.image = t('pswp__img', 'img')),
      this.g(),
    ...
    !this.heavyAppended &&
      ...
      (this.pswp.dispatch('appendHeavy', { slide: this }).defaultPrevented ||
        ...
        this.image &&
          ...
          this.image.complete
            ? this.M()
            : ((this.image.onload = () => this.M()),
              (this.image.onerror = () => this.M(!0))))));
  }
  v() {
    this.P || (this.container.appendChild(this.image), (this.P = !0));
  }
  M(t) {
    this.v(),
      (this.isLoading = !1),
  ...
  g() {
    if (!this.data.src || !this.width) return;
    const t = this.currentResolution || this.zoomLevels.initial;
    if (!t) return;
    const i = Math.round(this.width * t),
      s = Math.round(this.height * t);
    this.placeholder && f(this.placeholder, i, s);
    const { image: h } = this;
    h &&
      (f(h, i, s),
      (!h.dataset.largestUsedSize || i > h.dataset.largestUsedSize) &&
        ((h.sizes = i + 'px'), (h.dataset.largestUsedSize = i)),
  ...
  zoomTo(t, i, h, e) {
    ...
    o.animations.stopAllPan(), i || (i = o.getViewportCenterPoint());
    ...
    e || (t = n(t, this.zoomLevels.min, this.zoomLevels.max)),
      ...
      s(this.pan);
    const a = () => {
      this.T(t), this.applyCurrentZoomPan();
    };
    h
  ...
  calculateZoomToPanOffset(t, i, s) {
    const h = this.bounds.max[t] - this.bounds.min[t];
    let e;
    if (0 === h) return this.bounds.center[t];
    const n = this['x' === t ? 'width' : 'height'] * (s || this.currZoomLevel),
      { panPaddingRatio: o, panEdgeIsViewport: r } = this.pswp.options,
      a = this.panAreaSize[t] * o;
    let c;
    return (
      (c =
        !r && i[t] < a + this.pan[t]
          ? 0
          : !r && i[t] > n + this.pan[t] - a
          ? 1
          : (i[t] - this.bounds.min[t] - a) / (this.panAreaSize[t] - 2 * a)),
      (e = h * c + this.bounds.min[t]),
      (e = this.bounds.correctPan(t, e)),
      e
    );
  }
  panTo(t, i) {
  ...
  applyCurrentZoomPan() {
    this.C(this.pan.x, this.pan.y, this.currZoomLevel),
  ...
  _() {
    if (this.data.src && !this.loadError) {
      const { pswp: t } = this;
      i(this.panAreaSize, m(t.options, t.viewportSize)),
...
class _ {
  ...
  change() {
    ...
    if (
      ...
      if (!e.dispatch('verticalDrag', { panY: s }).defaultPrevented) {
        this.D('y', s, 0.6);
        const t = 1 - Math.abs(this.I(n.pan.y));
        e.applyBgOpacity(t), n.applyCurrentZoomPan();
      }
    } else {
      this.k('x') || (this.k('y'), s(n.pan), n.applyCurrentZoomPan());
    }
  }
  end() {
  ...
  k(t) {
    ...
    if (s.options.allowPanToNext && 'x' === h && 'x' === t && !n) {
      ...
      ('y' === t && (r.isShifted() || l.min.y === l.max.y)) || this.D(t, p);
  }
  I(t) {
    return (
...
class v {
  ...
  start() {
    (this.R = this.pswp.currSlide.currZoomLevel),
      i(this.Z, this.pswp.currSlide.pan),
  ...
  change() {
    ...
    g(this.O, i, e), g(this.L, t, s);
    let c = (1 / h(i, e)) * h(t, s) * this.R;
    if (
      (c > o.zoomLevels.initial + o.zoomLevels.initial / 15 && (this.B = !0),
      c < r)
    )
      if (n.options.pinchToClose && !this.B && this.R <= o.zoomLevels.initial) {
        const t = 1 - (r - c) / (r / 1.2);
        n.dispatch('pinchClose', { bgOpacity: t }).defaultPrevented ||
          n.applyBgOpacity(t);
      } else c = r - 0.25 * (r - c);
    else c > a && (c = a + 0.15 * (c - a));
    (o.pan.x = this.F('x', c)),
      (o.pan.y = this.F('y', c)),
      o.setZoomLevel(c),
    ...
    const { pswp: t } = this,
      { currSlide: i } = t;
    i.currZoomLevel < i.zoomLevels.initial && !this.B && t.options.pinchToClose
      ? t.close()
      : this.correctZoomPan();
  }
  F(t, i) {
    const s = i / this.R;
    return this.L[t] - (this.O[t] - this.Z[t]) * s;
  }
  correctZoomPan() {
    const { pswp: t } = this,
      { currSlide: s } = t;
    if (!s.isZoomable()) return;
    const h = s.currZoomLevel;
    let o,
      r = !0;
    ...
    const a = t.bgOpacity,
      ...
      l = i({}, s.pan);
    let p = i({}, l);
    r && (p = { x: this.F('x', o), y: this.F('y', o) }),
      s.setZoomLevel(o),
      (p = {
        x: s.bounds.correctPan('x', p.x),
        y: s.bounds.correctPan('y', p.y),
      }),
      s.setZoomLevel(h);
    let u = !0;
    if ((e(p, l) && (u = !1), !u && !r && !c))
      return s.T(o), void s.applyCurrentZoomPan();
      ...
      t.animations.startSpring({
        ...
        onUpdate: (i) => {
          if (((i /= 1e3), u || r)) {
            if (
              (u &&
                ((s.pan.x = l.x + (p.x - l.x) * i),
                (s.pan.y = l.y + (p.y - l.y) * i)),
              r)
            ) {
              const t = h + (o - h) * i;
              s.setZoomLevel(t);
          ...
          c && t.bgOpacity < 1 && t.applyBgOpacity(n(a + (1 - a) * i, 0, 1));
        },
        onComplete: () => {
          s.T(o), s.applyCurrentZoomPan();
        },
      });
  }
}
...
class x {
  ...
  click(t, i) {
    ...
    h ? this.V('imageClick', t, i) : e && this.V('bgClick', t, i);
  }
  tap(t, i) {
    y(i) && this.V('tap', t, i);
  }
  doubleTap(t, i) {
    y(i) && this.V('doubleTap', t, i);
  }
  V(t, i, s) {
...
class b {
  constructor(t) {
    (this.pswp = t),
      ...
      (this.N = 0),
      (this.q = []),
      (this.H = 'ontouchstart' in window),
      (this.j = !!window.PointerEvent),
      (this.supportsTouch = this.H || (this.j && navigator.maxTouchPoints > 1)),
      this.supportsTouch || (t.options.allowPanToNext = !1),
      (this.drag = new _(this)),
      (this.zoomLevels = new v(this)),
      (this.tapHandler = new x(this)),
      t.on('bindEvents', () => {
        t.events.add(t.scrollWrap, 'click', (t) => this.K(t)),
          this.j
            ? this.W('pointer', 'down', 'up', 'cancel')
            : this.H
            ? (this.W('touch', 'start', 'end', 'cancel'),
              (t.scrollWrap.ontouchmove = () => {}),
              (t.scrollWrap.ontouchend = () => {}))
            : this.W('mouse', 'down', 'up');
      });
  }
  W(t, i, s, h) {
    const { pswp: e } = this,
  ...
  onPointerDown(t) {
    ...
    h.opener.isOpen
      ? h.dispatch('pointerDown', { originalEvent: t }).defaultPrevented ||
        (s && (h.mouseDetected(), this.X(t)),
        h.animations.stopAll(),
        this.Y(t),
        (this.pointerDown = !0),
        1 === this.N && ((this.dragAxis = null), i(this.startP1, this.p1)),
        this.N > 1
          ? (this.J(), (this.isMultitouch = !0))
          : (this.isMultitouch = !1))
      : t.preventDefault();
  }
  onPointerMove(t) {
    ...
    this.N &&
      ...
      this.pswp.dispatch('pointerUp', { originalEvent: t }).defaultPrevented ||
        (0 === this.N &&
          ...
          this.isDragging
            ? this.$()
            : this.isZooming || this.isMultitouch || this.rt(t)),
        this.N < 2 &&
          this.isZooming &&
          ((this.isZooming = !1),
          this.zoomLevels.end(),
          1 === this.N && ((this.dragAxis = null), this.tt()))));
  }
  st() {
    (this.isDragging || this.isZooming) &&
      ...
      this.isDragging
        ...
        : e(this.p1, this.prevP1) ||
          e(this.p2, this.prevP2) ||
          this.zoomLevels.change(),
      this.at(),
      (this.raf = requestAnimationFrame(this.st.bind(this))));
  }
  ot(t) {
    const s = Date.now(),
      h = s - this.et;
    (h < 50 && !t) ||
      ((this.velocity.x = this.ct('x', h)),
      (this.velocity.y = this.ct('y', h)),
      (this.et = s),
      i(this.U, this.p1),
      (this.nt = !0));
  }
  rt(t) {
    ...
    this.lt
      ? (this.J(),
        h(this.G, this.startP1) < 25 &&
          this.tapHandler.doubleTap(this.startP1, t))
      : (i(this.G, this.startP1),
        (this.lt = setTimeout(() => {
          this.tapHandler.tap(this.startP1, t), this.J();
        }, e)));
  }
  J() {
    this.lt && (clearTimeout(this.lt), (this.lt = null));
  }
  ct(t, i) {
    const s = this.p1[t] - this.U[t];
  ...
  Y(t, s) {
    if (this.j) {
      const h = this.q.findIndex((i) => i.id === t.pointerId);
      s && h > -1
        ? this.q.splice(h, 1)
        : s || -1 !== h
        ? h > -1 && this.ut(t, this.q[h])
        : this.q.push(this.ut(t, {})),
        (this.N = this.q.length),
        this.N > 0 && i(this.p1, this.q[0]),
        this.N > 1 && i(this.p2, this.q[1]);
    } else
      (this.N = 0),
        t.type.indexOf('touch') > -1
          ? t.touches &&
            t.touches.length > 0 &&
            (this.ut(t.touches[0], this.p1),
            this.N++,
            t.touches.length > 1 && (this.ut(t.touches[1], this.p2), this.N++))
          : (this.ut(t, this.p1), s ? (this.N = 0) : this.N++);
  }
  at() {
    i(this.prevP1, this.p1), i(this.prevP2, this.p2);
  }
  tt() {
    i(this.startP1, this.p1), i(this.startP2, this.p2), this.at();
  }
  ht() {
...
class M {
  constructor(t) {
    (this.pswp = t), (this.x = 0), (this.slideWidth = 0), this.resetPosition();
  }
  resize(t) {
    const { pswp: i } = this;
    (this.slideWidth = Math.round(
      ...
      this.itemHolders.forEach((i, s) => {
        r(i.el, (s + this.dt) * this.slideWidth),
          t && i.slide && i.slide.resize();
      });
  }
  resetPosition() {
    (this.ft = 0), (this.wt = 0), (this.dt = -1);
  }
  appendHolders() {
    this.itemHolders = [];
    for (let i = 0; i < 3; i++) {
    ...
    if (
      (i
        ? (h.animations.startSpring({
            ...
            dampingRatio: 1,
            onUpdate: (t) => {
              (this.x = t), r(this.pswp.container, t);
            },
            onComplete: () => {
  ...
  getCurrSlideX() {
    return this.slideWidth * this.ft;
  }
  isShifted() {
  ...
  updateCurrItem() {
    ...
    (this.wt = this.ft),
      ...
      (t.currItemData = t.getItemData(t.currIndex));
    let s,
      h = Math.abs(i);
    h >= 3 && ((this.dt += i + (i > 0 ? -3 : 3)), (h = 3));
    for (let e = 0; e < h; e++)
      i > 0
        ? ((s = this.itemHolders.shift()),
          (this.itemHolders[2] = s),
          this.dt++,
          r(s.el, (this.dt + 2) * this.slideWidth),
          t.setContent(s, t.currIndex - h + e + 2))
        : ((s = this.itemHolders.pop()),
          this.itemHolders.unshift(s),
          this.dt--,
          r(s.el, this.dt * this.slideWidth),
          t.setContent(s, t.currIndex + h - e - 2));
    Math.abs(this.dt) > 50 &&
      !this.isShifted() &&
  ...
  moveTo(t, i) {
    ...
    !this.pswp.options.loop &&
      i &&
      ((s = (this.slideWidth * this.ft - t) / this.slideWidth),
...
class P {
  constructor(t) {
    (this.pswp = t),
      t.on('bindEvents', () => {
        t.options.initialPointerPos || this._t(),
          t.events.add(document, 'focusin', this.gt.bind(this)),
          t.events.add(document, 'keydown', this.vt.bind(this));
      });
    const i = document.activeElement;
    t.on('destroy', () => {
      t.options.returnFocus && i && this.yt && i.focus();
    });
  }
  _t() {
    this.yt || (this.pswp.template.focus(), (this.yt = !0));
  }
  vt(t) {
    const { pswp: i } = this;
  ...
  constructor(t) {
    ...
    (this.xt = i),
      ...
      (this.Pt = setTimeout(() => {
        a(i, o, e, n),
          (this.Pt = setTimeout(() => {
            i.addEventListener('transitionend', this.Mt, !1),
              i.addEventListener('transitioncancel', this.Mt, !1),
              (i.style[o] = r);
          }, 30));
      }, 0));
  }
  Mt(t) {
    t.target === this.xt && this.zt();
  }
  zt() {
    this.St || ((this.St = !0), this.onFinish(), this.bt && this.bt());
  }
  destroy() {
    this.Pt && clearTimeout(this.Pt),
      a(this.xt),
      this.xt.removeEventListener('transitionend', this.Mt, !1),
      this.xt.removeEventListener('transitioncancel', this.Mt, !1),
      this.St || this.zt();
  }
}
class S {
  constructor(t, i, s) {
    (this.velocity = 1e3 * t),
      (this.Tt = i || 0.75),
      (this.Ct = s || 12),
      this.Tt < 1 && (this.Dt = this.Ct * Math.sqrt(1 - this.Tt * this.Tt));
  }
  easeFrame(t, i) {
    let s,
      h = 0;
    i /= 1e3;
    const e = Math.E ** (-this.Tt * this.Ct * i);
    if (1 === this.Tt)
      (s = this.velocity + this.Ct * t),
        (h = (t + s * i) * e),
        (this.velocity = h * -this.Ct + s * e);
    else if (this.Tt < 1) {
      s = (1 / this.Dt) * (this.Tt * this.Ct * t + this.velocity);
      const n = Math.cos(this.Dt * i),
        o = Math.sin(this.Dt * i);
      (h = e * (t * n + s * o)),
        (this.velocity =
          h * -this.Ct * this.Tt + e * (-this.Dt * t * o + this.Dt * s * n));
    }
    return h;
  }
}
    ...
    const u = () => {
      this.kt &&
        ...
        Math.abs(p) < 1 && Math.abs(c.velocity) < 50
          ...
          : ((l = Date.now()), e(p + s), (this.kt = requestAnimationFrame(u))));
    };
    this.kt = requestAnimationFrame(u);
  }
  destroy() {
    this.kt >= 0 && cancelAnimationFrame(this.kt), (this.kt = null);
  }
}
class C {
  constructor() {
...
class D {
  constructor(t) {
    (this.pswp = t), t.events.add(t.template, 'wheel', this.Zt.bind(this));
  }
  Zt(t) {
    t.preventDefault();
    ...
    if (i)
      if (
        ...
        if (i.isZoomable()) {
          const s = i.currZoomLevel - 0.04 * i.zoomLevels.fit * h;
          i.zoomTo(s, { x: t.clientX, y: t.clientY });
        }
      } else i.isPannable() && i.panTo(i.pan.x - s, i.pan.y - h);
  }
}
class I {
  constructor(i, s) {
    ...
    'string' == typeof i.options[h + 'SVG'] && (s.html = i.options[h + 'SVG']),
      i.dispatch('uiElementCreate', { data: s });
    let e,
      n = 'pswp__';
    s.isButton && (n += 'button pswp__button--'),
      (n += s.class || s.name),
      s.isButton
        ? ((e = t(n, 'button')),
          (e.type = 'button'),
        ...
        : (e = t(n)),
      s.onInit && s.onInit(e, i),
      s.onClick &&
        (e.onclick = (t) => {
          'string' == typeof s.onClick ? i[s.onClick]() : s.onClick(t, e, i);
        });
    const o = s.appendTo || 'bar';
    let r;
    'bar' === o
      ? (i.topBar ||
          (i.topBar = t('pswp__top-bar pswp__hide-on-close', !1, i.scrollWrap)),
        (r = i.topBar))
      : (e.classList.add('pswp__hide-on-close'),
        (r = 'wrapper' === o ? i.scrollWrap : i.template)),
      r.appendChild(e);
  }
}
function k(t, i, s) {
...
class F {
  ...
  init() {
    const { pswp: t } = this;
    (this.isRegistered = !1),
      (this.uiElementsData = [E, A, Z, O, L, R]),
      ...
      t.on('zoomPanUpdate', () => this.Et());
  }
  registerElement(t) {
    this.isRegistered
      ? this.items.push(new I(this.pswp, t))
  ...
  Et() {
    ...
    if ((this.pswp.opener.isOpen || (h = i.zoomLevels.initial), h === this.Ot))
      return;
    this.Ot = h;
    const e = i.zoomLevels.initial - i.zoomLevels.secondary;
    if (Math.abs(e) < 0.01)
      return B(t, !1), void t.classList.remove('pswp--zoom-allowed');
...
class V {
  ...
  update(t) {
    ...
    const { preload: s } = i.options,
      h = void 0 === t || t >= 0;
    let e;
    for (e = 0; e <= s[1]; e++) this.Lt(i.currIndex + (h ? e : -e));
    for (e = 1; e <= s[0]; e++) this.Lt(i.currIndex + (h ? -e : e));
  }
  clearRecent() {
    this.Rt = [];
  }
  addRecent(t) {
    if (!(this.Rt.indexOf(t) > -1))
      return this.Rt.length > 14 && this.Rt.pop(), this.Rt.unshift(t), !0;
  }
  Lt(t) {
      ...
      this.addRecent(t) &&
        (function (t, i) {
          ...
          if (
            ...
            const { options: h } = i,
              e = m(h, i.viewportSize || d(h)),
              n = new u(h, s, t);
            n.update(s.w, s.h, e);
            const o = document.createElement('img');
            (o.decoding = 'async'),
              (o.sizes = Math.ceil(s.w * n.initial) + 'px'),
              s.srcset && (o.srcset = s.srcset),
              (o.src = s.src);
          }
        })(t, this.pswp);
  }
...
class U {
  ...
  close() {
    return (
      !(this.isClosed || this.isClosing || this.isOpening) &&
      ((this.isOpen = !1),
      (this.isOpening = !1),
      (this.isClosing = !0),
      (this.Ft = this.pswp.options.hideAnimationDuration),
      this.Vt(),
      setTimeout(
        () => {
          this.At();
  ...
  Bt() {
    this.pswp.off('firstZoomPan', this.Bt),
      this.isOpening ||
        ((this.isOpening = !0),
        (this.isClosing = !1),
        (this.Ft = this.pswp.options.showAnimationDuration),
        this.Vt());
  }
  Vt() {
    const { pswp: t } = this,
    ...
    if (
      ('fade' === s.showHideAnimationType
        ? ((s.showHideOpacity = !0), (this.Ut = !1))
        : 'none' === s.showHideAnimationType
        ? ((s.showHideOpacity = !1), (this.Ft = 0), (this.Ut = !1))
        : this.isOpening && t.Nt
        ? (this.Ut = t.Nt)
        : (this.Ut = this.pswp.getThumbBounds()),
      (this.qt = i.getPlaceholder()),
    ...
    this.jt && this.Ut.innerRect
      ? ((this.Gt = !0),
        (this.Yt = this.pswp.container),
        (this.Jt = this.pswp.currSlide.holderElement),
      ...
      this.isOpening
        ? (this.Wt &&
          ...
          this.Kt && ((t.template.style.opacity = 0.003), t.applyBgOpacity(1)),
          this.jt &&
            (this.Qt(),
            this.qt &&
              ((this.qt.willChange = 'transform'),
              (this.qt.style.opacity = 0.003))))
        : this.isClosing &&
  ...
  At() {
    this.isOpening && this.Ht && this.qt && 'IMG' === this.qt.tagName
      ? new Promise((t) => {
          let i = !1,
            s = !0;
          var h;
  ...
  $t() {
    this.pswp.template.style.setProperty(
      ...
      this.isOpening
        ? (this.qt && (this.qt.style.opacity = 1), this.ti())
        : this.isClosing && this.ii(),
      this.Ht || this.si();
  }
  si() {
    const { pswp: t } = this;
  ...
  ti() {
    ...
    this.jt &&
      (this.Gt &&
        (this.hi(this.Yt, 'transform', 'translate3d(0,0,0)'),
        this.hi(this.Jt, 'transform', 'none')),
      ...
      this.Wt && this.hi(t.bg, 'opacity', t.options.bgOpacity),
      this.Kt && this.hi(t.template, 'opacity', 1);
  }
  ii() {
    const { pswp: t } = this;
    this.jt && this.Qt(!0),
      this.Wt && t.bgOpacity > 0.01 && this.hi(t.bg, 'opacity', 0),
      this.Kt && this.hi(t.template, 'opacity', 0);
  }
  Qt(t) {
    const { pswp: s } = this,
      { innerRect: h } = this.Ut,
    ...
    if (this.Gt) {
      const i = -n.x + (this.Ut.x - h.x) + h.w,
        s = -n.y + (this.Ut.y - h.y) + h.h,
        e = n.x - h.w,
        a = n.y - h.h;
      t
        ? (this.hi(this.Yt, 'transform', o(i, s)),
          this.hi(this.Jt, 'transform', o(e, a)))
        : (r(this.Yt, i, s), r(this.Jt, e, a));
    }
    i(e.pan, h || this.Ut),
      (e.currZoomLevel = this.Ut.w / e.width),
      t
        ? this.hi(e.container, 'transform', e.getCurrentTransform())
        : e.applyCurrentZoomPan();
  }
  hi(t, i, s) {
    if (!this.Ft) return void (t.style[i] = s);
    const { animations: h } = this.pswp,
      e = {
        duration: this.Ft,
        easing: this.pswp.options.easing,
        onComplete: () => {
          h.activeAnimations.length || this.si();
        },
        target: t,
      };
...
export default class extends class extends class {
  ...
  off(t, i) {
    this.ei[t] && (this.ei[t] = this.ei[t].filter((t) => i !== t)),
  ...
  getNumItems() {
    ...
    i
      ? i.length
        ? (t = i.length)
        : i.gallery &&
          (i.items || (i.items = this.ni(i.gallery)),
  ...
  getItemData(t) {
    ...
    Array.isArray(i)
      ...
      : i &&
        i.gallery &&
        (i.items || (i.items = this.ni(i.gallery)), (s = i.items[t]));
    let h = s;
    h instanceof Element && (h = this.oi(h));
  ...
  constructor(t, i) {
    super(),
      ...
      (this.gestures = new b(this)),
      (this.opener = new U(this)),
      (this.keyboard = new P(this)),
      (this.lazyLoader = new V(this));
  }
  init() {
    ...
    return (
      ...
      this.dispatch('gettingData', this.currIndex, this.currItemData, !0),
      (this.Nt = this.getThumbBounds()),
      ...
      this.on('initialZoomInEnd', () => {
        this.setContent(this.mainScroll.itemHolders[0], this.currIndex - 1),
          ...
          this.events.add(window, 'resize', this.pi.bind(this)),
          this.events.add(window, 'scroll', this.di.bind(this)),
  ...
  updateSize(t) {
    ...
    const s = d(this.options, this);
    (!t && e(s, this.ai)) ||
      (i(this.ai, s),
      this.dispatch('beforeResize'),
      i(this.viewportSize, this.ai),
      this.di(),
  ...
  getThumbBounds() {
    ...
    })(this.currIndex, this.currItemData, this);
  }
  ci() {
    (this.plugins = {}),
      this.pluginClasses &&
        Object.keys(this.pluginClasses).forEach((t) => {
          const i = this.pluginClasses[t];
          'function' == typeof i && (this.plugins[t] = new i(this));
        });
  }
  ri(t) {
